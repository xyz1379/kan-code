...
class LatentSelfAttentionKAN(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int, bottleneck_size: int):
        ...
        self.qkv_proj = KANLayer(embed_dim, embed_dim * 3, k=3)
        self.out_proj = KANLayer(embed_dim, embed_dim, k=3)
...
class FullTransformerKAN(nn.Module):
    ...
    def __init__(self, vocab_size, emb_dim, latent_dim, latent_len, num_heads, mlp_dim, num_layers, num_classes, latent_bottleneck):
        ...
        self.embed_to_latent = KANLayer(emb_dim, latent_dim, k=3)
...
plt.title('Training Loss vs Epochs (k=3)', fontsize=14)
...
plt.title('Validation Accuracy vs Epochs (k=3)', fontsize=14)
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
with open(report_path, "w") as f:
    f.write("Classification Report - Test Set (k=3)\n")
    f.write("=" * 80 + "\n\n")
    f.write(report)
...
plt.title('Confusion Matrix Heatmap - Test Set (k=3)', fontsize=15, fontweight='bold', pad=20)
