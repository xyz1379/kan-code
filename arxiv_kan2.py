...
# Must have pykan installed: pip install pykan
...
class MLAWithRoPEKAN(nn.Module):
    ...
    def __init__(self, input_dim, latent_dim, latent_len, num_heads):
        ...
        self.q_proj = KANLayer(latent_dim, latent_dim, k=2)
        self.k_proj = KANLayer(input_dim, latent_dim, k=2)
        self.v_proj = KANLayer(input_dim, latent_dim, k=2)
...
class LatentSelfAttentionKAN(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int, bottleneck_size: int):
        ...
        self.qkv_proj = KANLayer(embed_dim, embed_dim * 3, k=2)
        self.out_proj = KANLayer(embed_dim, embed_dim, k=2)
...
class TransformerMLABlockKAN(nn.Module):
    ...
    def __init__(self, latent_dim, num_heads, mlp_dim, latent_bottleneck):
        ...
        self.fc1 = KANLayer(latent_dim, mlp_dim, k=2)
        self.fc2 = KANLayer(mlp_dim, latent_dim, k=2)
...
class FullTransformerKAN(nn.Module):
    ...
    def __init__(self, vocab_size, emb_dim, latent_dim, latent_len, num_heads, mlp_dim, num_layers, num_classes, latent_bottleneck):
        ...
        self.embed_to_latent = KANLayer(emb_dim, latent_dim, k=2)
        ...
        self.classifier = KANLayer(latent_dim * latent_len, num_classes, k=2)
...
num_heads, mlp_dim, num_layers = 8, 1024, 6 
latent_bottleneck = 8
...
plt.title('Training Loss vs Epochs (k=2)', fontsize=14)
...
plt.title('Validation Accuracy vs Epochs (k=2)', fontsize=14)
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
with open(report_path, "w") as f:
    f.write("Classification Report - Test Set (k=2)\n")
    f.write("=" * 80 + "\n\n")
    f.write(report)
...
plt.title('Confusion Matrix Heatmap - Test Set (k=2)', fontsize=15, fontweight='bold', pad=20)
