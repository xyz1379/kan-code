...
class OptimizedMLAWithRoPE(nn.Module):
    ...
    def __init__(self, input_dim, latent_dim, latent_len, num_heads):
        ...
        # <--- MODIFIED: Pure Linear Projections (MLP in Heads) ---
...
# <--- MODIFIED: HYBRID FFN (2 KAN, 6 MLP) ---
num_kan_experts = 2
num_mlp_experts = 6
...
for epoch in range(num_epochs):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), "best_model_arxiv.pt")
...
print("\n=== Final Test ===")
model.load_state_dict(torch.load("best_model_arxiv.pt"))
...
plt.savefig("confusion_matrix_moe_arxiv.png")
plt.show()
...
plt.tight_layout()
plt.savefig("training_plot_moe_arxiv.png")
plt.show()